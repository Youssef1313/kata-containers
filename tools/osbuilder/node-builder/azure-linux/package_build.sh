...
if [ "${CONF_PODS}" == "no" ]; then
	runtime_make_flags+=" DEFMEMSZ=64 DEFSTATICRESOURCEMGMT_CLH=true KERNELPATH_CLH=${KERNEL_BINARY_LOCATION}"
else
	runtime_make_flags+=" DEFMEMSZ=160 CLHPATH=${CLOUD_HYPERVISOR_LOCATION}"
fi
